...
{
    std::cout << "Welcome to the Bingo Deck Generator" << std::endl;
    ...
    while (keepGoing)
    {
        ...
        else if (command=="D")
        {
            if (currentDeck) {
                ...
                std::cout<<"You do not currently have any decks."<<std::endl;
            }
            currentDeck = nullptr;
        }
        else if (command=="X")
...
void UserInterface::deckMenu()
{
    ...
    while (keepGoing)
    {
        ...
        {
            std::cout << std::endl;
            currentDeck->printDeck(std::cout);
        }
        else if (command=="S")
        {
            saveDeck();
...
{
    int cardToPrint = getNumberInput("Id of card to print", 1, currentDeck->getNumCards());
    if (cardToPrint>0)
    {
        std::cout << std::endl;
        currentDeck->printCard(std::cout, cardToPrint);
    }
}
...
void UserInterface::saveDeck()
{
    ...
    if (fileName!="")
    {
        ...
        currentDeck->printDeck(outputStream);
